        ...
        scope.launch {
            try {
                ...
                if (liveSnapshot.packageName != request.packageName ||
                    liveSnapshot.looseState != request.looseState
                ) {
                    DatabaseHelper.logAction(
                        context = this@AutoAccessibilityService,
                        ...
                        actionDetail = "[STALE:$source] ${request.command.target}",
                        success = false
                    )
                ...
                if (!SafetyChecker.checkCooldown()) {
                    DatabaseHelper.logAction(
                        ...
                        state = request.stableState,
                        actionType = "${request.command.type.name}:REMOTE",
                        actionDetail = "[COOLDOWN:$source] ${request.command.target}",
                        success = false
                    )
                    return@launch
                }
                ...
                val success = ActionExecutor.executeSafe(this@AutoAccessibilityService, request.command)
                ...
                DatabaseHelper.logAction(
                    ...
                    state = request.stableState,
                    actionType = "${request.command.type.name}:REMOTE",
                    actionDetail = "[$source] ${request.command.target}",
                    success = success
                )

                if (success) {
                    lastAssistActionState = request.looseState
                    lastAssistActionTarget = request.command.target
                    lastAssistActionTime = System.currentTimeMillis()
                    OverlayService.updateStatus("REMOTE ${request.command.type.name}: ${request.command.target}")
                }
            } catch (ce: CancellationException) {
                throw ce
            } catch (e: Exception) {
